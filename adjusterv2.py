...
def detect_break(series: pd.Series, event_date: pd.Timestamp,
    ...
    # Fetch one row before lo to:
    #   (a) seed ffill so #N/A at the window boundary gets filled
    #   (b) compute the step that straddles lo (e.g. 4/16→4/17 when lo=4/17)
    # The seed row is retained during step calculation then excluded when
    # returning the break date (we only return dates within [lo, hi]).
    ...
    # Do NOT trim seed yet — we need it to compute the lo-boundary step
    ...
    for i in range(1, len(window)):
        ...
        if dist < best_dist:
            best_dist = dist
            best_step = step
            # break_date = the first date AFTER the jump; must be within [lo, hi]
            candidate = window.index[i]
            best_date = candidate if candidate >= lo else lo
